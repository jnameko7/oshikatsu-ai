...
(function(){
  ...
  async function loadArticles(){
    try{
      const data=await fetch('/api/articles',{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null);
  ...
  function cardHtml(article){
    ...
    return `<a class="article-card blog-card" href="${escapeAttr(href)}">${img?`<img src="${escapeAttr(img)}" alt="${escapeAttr(title)}">`:''}<div class="article-card-body blog-card-body"><span class="article-category">${escapeHtml(category)}</span><h2>${escapeHtml(title)}</h2>${date?`<p class="article-date">${escapeHtml(formatDate(date))}</p>`:''}${desc?`<p>${escapeHtml(desc)}</p>`:''}</div></a>`;
  }
  ...
  function normalizeSlug(v){return decodeURIComponent(String(v||'')).trim().replace(/^https?:\/\/[^/]+/,'').replace(/^\/?article\.html\/?/i,'').replace(/^\/?article\/?/i,'').replace(/^\?id=/i,'').replace(/^\?slug=/i,'').replace(/^\/+|\/+$/g,'').replace(/\.html?$/i,'').toLowerCase();}
  function formatDate(v){const d=new Date(v);return isNaN(d)?String(v||''):d.toLocaleDateString('ja-JP',{year:'numeric',month:'long',day:'numeric'});}
