...
function renderArticle(article, pool) {
  ...
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}｜推し活資金AI</title><meta name="description" content="${attr(desc)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${attr(canonical)}"><meta property="og:type" content="article"><meta property="og:title" content="${attr(title)}"><meta property="og:description" content="${attr(desc)}"><meta property="og:url" content="${attr(canonical)}">${image ? `<meta property="og:image" content="${attr(image)}">` : ""}<link rel="stylesheet" href="/style.css">${analytics()}<script async src="https://pagead2.googlesyndication.com/pagead/js?client=ca-pub-6088406710027099" crossorigin="anonymous"></script><script type="application/ld+json">${json(articleLd)}</script><script type="application/ld+json">${json(breadcrumbLd)}</script><style>${articleCss()}</style></head><body>${header()}<main class="ssr-article"><article>${image ? `<img class="ssr-eye" src="${attr(image)}" alt="${attr(title)}" fetchpriority="high">` : ""}<div class="ssr-inner"><nav class="ssr-breadcrumb"><a href="/">ホーム</a><span>›</span><a href="/articles">お役立ち記事</a><span>›</span><span>${esc(title)}</span></nav><div class="ssr-category">♡ ${esc(category)}</div>${tags.length ? `<div class="ssr-tags">${tags.map(t => `<a href="/articles?tag=${encodeURIComponent(t)}">${esc(t)}</a>`).join("")}</div>` : ""}<h1>${esc(title)}</h1><div class="ssr-meta">${published ? `<span>公開日：${date(published)}</span>` : ""}${updated ? `<span>更新日：${date(updated)}</span>` : ""}<span>読了目安：約${readMinutes}分</span></div><section class="ssr-learn"><h2>この記事でわかること</h2><ul>${headings(bodyWithIds).map(h => `<li>${esc(h)}</li>`).join("")}</ul></section>${toc}<div class="ssr-body">${bodyWithTool}</div><section class="ssr-verify"><h2>情報の確認方法</h2><p>料金・時刻・在庫・販売条件など変わりやすい情報は、予約・購入前に必ず公式サイトで再確認してください。当サイトは公式情報を優先して確認し、記事の更新日を表示しています。詳しい基準は<a href="/editorial-policy">編集方針</a>と<a href="/methodology">調査方法</a>で公開しています。</p></section><section class="ssr-tool-bottom"><div><span>読んだあとに試せる無料ツール</span><h2>${esc(tool.name)}</h2><p>${esc(tool.description)}</p></div><a href="${tool.href}">${esc(tool.button)} →</a></section><section class="ssr-next-steps"><h2>予算づくりを次の行動につなげる</h2><div><a href="/fund"><b>1</b><span><strong>使える金額を知る</strong>推し活資金診断</span></a><a href="/transport"><b>2</b><span><strong>遠征費を見積もる</strong>遠征プランナー</span></a><a href="/kakeibo"><b>3</b><span><strong>使った金額を記録</strong>推し活家計簿</span></a><a href="/yearly"><b>4</b><span><strong>次の目標を決める</strong>目標シミュレーター</span></a></div></section><section class="ssr-author"><h2>この記事を書いた人</h2><h3>推し活資金AI編集部</h3><p>ライブ遠征、ホテル、交通費、チケット、夜行バス、推し活の節約方法を中心に、初心者にも実践しやすい情報を発信しています。公式サイトや運営会社の情報を確認し、必要に応じて内容を見直しています。</p><a href="/about">編集方針・運営者情報を見る →</a></section>${related.length ? `<section class="ssr-related"><h2>あわせて読みたい記事</h2><div>${related.map(relatedCard).join("")}</div></section>` : ""}</div></article></main>${footer()}${stickyTools(tool)}${affiliateCardsScript()}</body></html>`;
}
