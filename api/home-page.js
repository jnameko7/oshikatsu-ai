...
export default async function handler(req, res) {
  try {
    ...
    html = replaceMarker(
      html,
      "SSR_LATEST",
      articles.length ? articles.slice(0, 8).map(renderLatest).join("") : fallbackLatest()
    );
...
function renderRanking(article, index) {
  ...
  return `<a class="topv3-rank-item" href="${href}"><span>${index + 1}</span>${thumb}<div><h3>${esc(title)}</h3><p>${(tags.length ? tags : ["推し活", "お役立ち"]).map(t => `<b>${esc(t)}</b>`).join("")}</p></div><i></i></a>`;
}
