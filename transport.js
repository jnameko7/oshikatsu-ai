q("transportForm")?.addEventListener("submit",async e=>{
  e.preventDefault();
  const from=q("fromStation").value.trim();
  const to=q("toStation").value.trim();
  const type=q("transportType").value;
  const fare=num("fare");
  const hotel=num("hotel");
  const food=num("foodCost");
  const local=num("localCost");
