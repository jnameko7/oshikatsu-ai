...
function buildTransportCards(fareResult){
  if(typeof allTransportFareCardsHTML==="function")return allTransportFareCardsHTML(fareResult);
  return `<div class="budget-cards"><div class="budget-card"><span>夜行・高速バス</span><b>${yenBudget(fareResult?.bus?.normal||6500)}</b><small>目安</small></div></div>`;
}
...
function buildPlanner(){
  ...
  const sourceLabel=fareResult.source==="database"?"主要路線データ":"距離計算による推定料金";
  const confidenceLabel=fareResult.confidence==="high"?"信頼度：高":"信頼度：低 / 実料金と大きく異なる場合があります";
  const status=shortage>0?`<p>予算を ${yenBudget(shortage)} 超えています。交通手段・宿泊数・食費を見直してください。</p>`:`<p>グッズ代に使える想定残額は ${yenBudget(goods)} です。</p>`;
  result.classList.add("show");
  result.innerHTML=`<div class="result-main"><p>${from} → ${to} の遠征予算プラン</p><strong>グッズ代 ${yenBudget(goods)}</strong>${status}<p style="margin-top:10px;color:#6e7587">${sourceLabel} / ${confidenceLabel}</p></div>
  <div class="budget-cards"><div class="budget-card"><span>交通費目安</span><b>${yenBudget(transportTotal)}</b><small>${transportName(chosen.type)} / ${people}人分</small></div><div class="budget-card"><span>宿泊費提案</span><b>${yenBudget(hotelTotal)}</b><small>${hotelMessage}</small></div><div class="budget-card"><span>理想食費</span><b>${yenBudget(meal.total)}</b><small>${meal.detail}</small></div></div>
  ...
  <div class="result-summary"><h2>予算配分</h2><ul><li><span>遠征予算</span><b>${yenBudget(budget)}</b></li><li><span>交通費</span><b>${yenBudget(transportTotal)}</b></li><li><span>宿泊費</span><b>${yenBudget(hotelTotal)}</b></li><li><span>食費</span><b>${yenBudget(meal.total)}</b></li><li><span>予備費</span><b>${yenBudget(reserve)}</b></li><li><span>最低確保したいグッズ代</span><b>${yenBudget(goodsMin)}</b></li><li><span>グッズ代に使える残額</span><b>${yenBudget(goods)}</b></li></ul></div>
  <div class="result-summary"><h2>交通手段別の目安</h2>${buildTransportCards(fareResult)}<p style="color:#6e7587;font-weight:800;margin-top:12px">${fareResult.note||""}</p></div>${buildTransportNotice(fareResult)}`;
