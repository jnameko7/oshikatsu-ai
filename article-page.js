...
function renderArticle(article, list) {
  ...
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}｜推し活資金AI</title><meta name="description" content="${attr(desc)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${attr(canonical)}"><meta property="og:type" content="article"><meta property="og:title" content="${attr(title)}"><meta property="og:description" content="${attr(desc)}"><meta property="og:url" content="${attr(canonical)}">${image ? `<meta property="og:image" content="${attr(image)}">` : ""}<link rel="stylesheet" href="/style.css">${analytics()}<!-- AdSense review mode --><script type="application/ld+json">${json(articleLd)}</script><script type="application/ld+json">${json(breadcrumbLd)}</script><style>${articleCss()}</style></head><body>${header()}<main class="ssr-article"><article>${image ? `<img class="ssr-eye" src="${attr(image)}" alt="${attr(title)}" loading="eager">` : ""}<div class="ssr-inner"><nav class="ssr-breadcrumb"><a href="/">ホーム</a><span>›</span><a href="/articles">お役立ち記事</a><span>›</span><span>${esc(title)}</span></nav><div class="ssr-category">♡ ${esc(category)}</div><h1>${esc(title)}</h1><div class="ssr-meta">${published ? `<span>公開日：${date(published)}</span>` : ""}${updated ? `<span>更新日：${date(updated)}</span>` : ""}<span>読了目安：約${readMinutes(body)}分</span></div><section class="ssr-learn"><h2>この記事でわかること</h2><ul>${headings(body).map(h => `<li>${esc(h)}</li>`).join("")}</ul></section><div class="ssr-body">${body}</div><section class="ssr-experience"><h2>20年以上推し活を続ける運営者から</h2><p>運営者はこれまで50回以上のライブ・イベント参加と、宿泊を伴う遠征を経験しています。遠征先で旅行気分になり、予定以上に使ってしまった経験も踏まえ、読者が自分のペースで推し活を続けられるよう、実践しやすさを重視して記事を編集しています。</p><p><strong>情報の区別：</strong>運営者が実際に経験した内容は体験として記載し、未体験のサービスや施設は公式情報を確認した調査内容として紹介します。</p></section><section class="ssr-check"><h2>この記事の確認方針</h2><dl><div><dt>最終確認日</dt><dd>${updated ? date(updated) : (published ? date(published) : "公開時")}</dd></div><div><dt>確認内容</dt><dd>公式サイト、運営会社、料金・利用条件などを必要に応じて確認</dd></div></dl><p>料金や販売条件は変更される場合があります。予約・購入前に必ず公式サイトの最新情報をご確認ください。</p></section><section class="ssr-author"><h2>この記事を書いた人</h2><h3>推し活資金AI編集部</h3><p>運営者は推し活歴20年以上、ライブ・イベント参加50回以上、遠征経験あり。実体験と公式情報による調査を区別し、初心者にも実践しやすい記事を発信しています。</p><a href="/about">編集方針・運営者情報を見る →</a></section>${related.length ? `<section class="ssr-related"><h2>あわせて読みたい記事</h2><div>${related.map(card).join("")}</div></section>` : ""}</div></article></main>${footer()}${stickyBanner(tool)}${stickyScript()}</body></html>`;
}
